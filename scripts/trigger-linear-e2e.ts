...
async function main() {
  console.log(`\n=== 1. POST ${BASE_URL}/api/analyses/linear ===`);
  const startTime = Date.now();

  const res = await fetch(`${BASE_URL}/api/analyses/linear`, {
    method: "POST",
    headers: {
  ...
  if (!res.ok || !json.id) {
    console.error("Endpoint failed or returned no analysis ID:", json);
    process.exit(1);
  }

  const analysisId = json.id;
  console.log(`\n=== 2. Polling status for Analysis ID: ${analysisId} ===`);
  ...
  console.log(`\nAnalysis finished in ${elapsed}s! Status: ready`);
  ...
  console.log(`   HTTP ${httpStatus} ${httpStatusText}`);
  console.log(`   Payload: ${JSON.stringify(json)}`);
  console.log("\n================ END OF REPORT ================\n");
}

main().catch((err) => {
  console.error("Pilot execution script error:", err);
  process.exit(1);
});
